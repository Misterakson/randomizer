...
class textUpdate
{
    ...
    private $search = array('@<script[^>]*?>.*?</script>@si',  // Strip out javascript
    ...
    public function __construct($text,$rand){
        $this->rand = $rand;
        $this->text = preg_split($this->search[1],$text,null,PREG_SPLIT_NO_EMPTY);
//        print_r($this->text[0]);
//        if(mb_strlen($this->text[0])<=60){
//            $arr = explode(' ',$this->text[0]);
//            $arr[0] = "<b>".$arr[0];
//            $arr[count($arr)-1] = $arr[count($arr)-1]."</b>";
//            $res = implode(' ',$arr);
//        }
//        print $res;
//        print mb_strlen($this->text[0]);
//        die();
    }

    public function del_symb()
    {
        foreach ($this->text as $sentence=>$text)
        {
//            $this->text[$sentence] = $text."asdasf";

            for($i = 0; $i<mb_strlen($this->text[$sentence]);$i++)
            {
                if($this->rand > rand(0,99))
                {
                    $this->text[$sentence] = iconv('UTF-8', 'windows-1251', $sentence);
                    $sentence[$i] = str_replace($sentence[$i],'',$sentence[$i]);
                    $sentence = iconv('windows-1251', 'UTF-8', $sentence);
                }
            }
...
//    public function del_symb($this->rand){
//
